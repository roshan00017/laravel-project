                ...
                <table style="background-color: #f2f3f8; max-width:670px;  margin:0 auto;" width="100%" border="0"
                    ...
                    <tr>
                        <td style=" padding-top: 17px; font-size: 23px;"><strong
                                class="{{ setFont() }}">{{ trans('agenda.greeting') }}
                                @if (isset($memberName))
                                    {{ $memberName }}
                                @endif
                            </strong></td>
                    ...
                    <tr>
                        <td style="text-align:left; padding-top: 5px;" class="{{ setFont() }}">
                    ...
                    <tr>
                        <td style="text-align:left; padding-top: 5px;" class="{{ setFont() }}">
                            ...
                            <strong class="{{ setFont() }}">
                                @if (isset($data->meeting->meeting_date_ad))
                                    {{ getLan() == 'np' ? $data->meeting->meeting_date_bs : $data->meeting->meeting_date_ad }}
                                @endif
                            </strong>
                        </td>
                    </tr>
                    <tr>
                        <td style="text-align:left; padding-top: 5px;" class="{{ setFont() }}">
                            ...
                            <strong class="{{ setFont() }}">
                                @if ($data->meeting->meeting_mode == 'online')
                                    @if (isset($data->meeting->meeting_url))
                                        <a
                                            href="{{ $data->meeting->meeting_url }}">{{ $data->meeting->meeting_url }}</a>
                                        {{-- {{ $data->meeting->meeting_url }} --}}
                                    @endif
                                @else
                            ...
                            <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0"
                                ...
                                <tbody>
                                    @foreach ($agendaList as $key => $data)
                                        <tr>
                                            <th scope=row {{ setFont() }} style="padding-bottom: 14px">
                                                {{ ++$key }}
                                            </th>
