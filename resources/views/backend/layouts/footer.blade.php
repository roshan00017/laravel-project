<footer class="main-footer">
    ...
    <strong class="{{setFont()}}">
        ...
        <a href="{{url('/dashboard')}}">@if(systemSetting()->app_name) {{getLan() == 'np' ? systemSetting()->app_name_np : systemSetting()->app_name}} @else {{ env('APP_NAME') }}  @endif </a>
   ...
   {{-- <script src="{{asset('plugins/jquery/jquery.min.js')}}"></script>--}}
    ...
    <script type="text/javascript">
        ...
        function addMore(weekDays) {
            count++;
            let options = '<option value="">{{trans('calendar.week_day').' '.trans('calendar.select')}}</option>';
            ...
            let deleteBtn = '<div class="form-group col-md-4 {{setFont()}}" id="deleteBtn'+count+'">' +
                '<a class="btn btn-danger rounded-pill {{setFont()}}" title="{{trans('calendar.add_more')}}">' +
                '<i class="fa fa-minus-circle"></i></a>' +
                '</div>';

            $('#wrapper').append('<div class="row" id="newField'+count+'">' + selectWeekDayField + dayField + deleteBtn + '</div>');
