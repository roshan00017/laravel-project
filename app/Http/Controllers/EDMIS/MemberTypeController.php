...
class MemberTypeController extends BaseController
{
    ...
    public function __construct(MemberType $memberType, LogsRepository $logsRepository)
    {
        parent::__construct();
        $this->model = new CommonRepository($memberType);
     ...
     {
         try {
             $data['memberType'] = MemberType::all();
             $data['page_url'] = 'membertypes';
             $data['page_route'] = 'membertypes';
             $data['load_js'] = [
                 'js/custom_app.min.js',

             ];
             $data['page_title'] = getLan() == 'np' ? 'सदस्य प्रकार' : 'Members Type';
             ...
             return view('backend.edmis.memberType.index', $data);
     ...
     public function store(Request $request): RedirectResponse
     {
         try {
             ...
             $data = $request->all();
             $checkData = MemberType::query()->orderBy('id', 'desc')->select('code')->first();
