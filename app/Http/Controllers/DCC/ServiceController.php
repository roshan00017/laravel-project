...
use App\Http\Requests\Dcc\CommonRequest;
...
class ServiceController extends BaseController
{
    ...
    public function __construct(Service $service, LogsRepository $logsRepository)
    {
        parent::__construct();
    ...
    {
        try {
            $data['serviceType'] = Service::all();
            $data['page_url'] = 'services';
            $data['page_route'] = 'services';
            $data['load_js'] = [
                'js/custom_app.min.js',

            ];
            $data['page_title'] = getLan() == 'np' ? 'सेवा' : 'Service';
            ...
            return view('backend.dcc.service.index', $data);
    ...
    public function store(CommonRequest $request): RedirectResponse
    {
        try {
    ...
    public function update(CommonRequest $request, $id): RedirectResponse
    {
        try {
            DB::beginTransaction();
   ...
   public function status($id): RedirectResponse
   {
    ...
    public function show($id)
    {
        ...
        try {
            ...
            if ($hashIdValue) {
                $value = Service::query()->find($hashIdValue[0]);
                // Assuming you have a User model and a users table in your database
                $data['dcDispatchBook'] = Service::find($value->id);
                if ($data) {

                    $data['page_title'] = getLan() == 'np' ? 'सेवा' : 'Service';
                    $data['page_url'] = 'service';
                    $data['page_route'] = 'service';
                    // User found, display the details

                    return view('backend.dcc.service.show', $data);
                }
