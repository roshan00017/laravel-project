...
class ScheduleTypeController extends BaseController
{
    ...
    public function __construct(
        ...
        parent::__construct();
        // set the model
        $this->model = new CommonRepository($scheduleType);
    ...
    {
        try {
            $data['page_url'] = 'scheduleType';
            $data['page_route'] = 'scheduleType';
            $data['results'] = $this->model->all();
            $data['page_title'] = getLan() == 'np' ? 'तालिका प्रकार' : 'Schedule Type';
            // dd($data);
            $data['load_js'] = [
                'js/custom_app.min.js',
            ];

            return view('backend.appointment.scheduleType.index', $data);
        } catch (\Exception $e) {
