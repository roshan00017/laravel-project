...
use App\Http\Requests\VisitingPurposeRequest;
...
class VisitingPurposeController extends BaseController
{
    protected CommonRepository $model;

    protected LogsRepository $logsRepository;

    private int $menuId = 78;

    public function __construct(
        VisitingPurpose $model,
    ...
    {
        try {
            $data['page_url'] = 'basicDetails/vistingPurposes';
            $data['page_route'] = 'vistingPurposes';
            $data['page_title'] = getLan() == 'np' ? 'भेटघाट उद्देश्य' : 'Visiting Purpose';
            ...
            return view('backend.basicDetails.visitingPurpose.index', $data);
        } catch (\Exception $e) {
            dd($e);
     ...
     */
    public function store(VisitingPurposeRequest $request): RedirectResponse
    {
        try {
            $data = $request->all();
            DB::beginTransaction();
