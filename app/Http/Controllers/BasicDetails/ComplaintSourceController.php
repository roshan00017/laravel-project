...
use App\Http\Requests\ComplaintSourceRequest;
...
class ComplaintSourceController extends BaseController
{
    ...
    public function __construct(ComplaintSource $model, LogsRepository $logsRepository)
    {
        parent::__construct();
    ...
    {
        try {
            $data['page_url'] = 'basicDetails/complaintSource';
            $data['page_route'] = 'complaintSource';
            $data['results'] = $this->model->all($request);
            $data['page_title'] = getLan() == 'np' ? 'गुनासो स्रोत' : 'Complaint Source';
            $data['request'] = $request;
            $data['load_js'] = [
                'js/custom_app.min.js',
            ];

            return view('backend.basicDetails.complaintSource.index', $data);
        } catch (\Exception $e) {
            dd($e);
    ...
    public function store(ComplaintSourceRequest $request): RedirectResponse
    {
