...
class DateConverter
{
     ...
     * @return array
     */
    public function eng_to_nep($req_date, $format = null, $dayFormat = null)
    {
        if (! $req_date) {
            return '-';
        }
        $yy = date('Y', strtotime($req_date));
        $mm = date('m', strtotime($req_date));
        $dd = date('d', strtotime($req_date));
        ...
        if ($chk !== true) {
            ...
            $this->_nep_date['np_day'] = $this->day_eng_nep($this->_get_day_of_week($day));
            $this->_nep_date['num_day'] = $numDay;
            if ($format) {
                return $this->_nep_date['np_day'].', '.$this->_nep_date['nmonth'].' '.$total_nDays.' '.$y.' ';
            } elseif ($dayFormat) {
                return $total_nDays.' '.$this->_nep_date['nmonth'].' '.$y.', '.$this->_nep_date['np_day'];
            } else {
                return $y.'-'.str_pad($m, 2, '0', STR_PAD_LEFT).'-'.str_pad($total_nDays, 2, '0', STR_PAD_LEFT);
            }
        }
    }
    ...
    public function day_eng_nep($day)
    {
        switch ($day) {
            case 'Sunday':
                // $day = "Sunday";
